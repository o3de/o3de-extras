...
namespace ROS2Controllers {
    void RigidBodyTwistControlComponent::Reflect(AZ::ReflectContext *context) {
        ...
        if (AZ::SerializeContext *serialize = azrtti_cast<AZ::SerializeContext *>(context)) {
            serialize->Class<RigidBodyTwistControlComponent, AZ::Component>()
                    ->Version(1)
                    ->Field("Config", &RigidBodyTwistControlComponent::m_config);
            if (AZ::EditContext *ec = serialize->GetEditContext()) {
                ec->Class<RigidBodyTwistControlComponent>("Rigid Body Twist Control",
                                                          "Simple control through RigidBody")
    ...
    void RigidBodyTwistControlComponent::OnTick([[maybe_unused]] float deltaTime,
    ...
    void RigidBodyTwistControlComponent::OnSceneSimulationFinish(AzPhysics::SceneHandle sceneHandle,
        ...
        if (m_config.m_physicalApi == RigidBodyTwistControlComponentConfig::PhysicalApi::Velocity) {
            ...
            const auto linearVelocityGlobal = robotTransform.TransformVector(m_linearVelocityLocal);
            const auto angularVelocityGlobal = robotTransform.TransformVector(m_angularVelocityLocal);
            Physics::RigidBodyRequestBus::Event(GetEntityId(), &Physics::RigidBodyRequests::SetLinearVelocity,
                                                linearVelocityGlobal);
            Physics::RigidBodyRequestBus::Event(GetEntityId(), &Physics::RigidBodyRequests::SetAngularVelocity,
                                                angularVelocityGlobal);
            return;
        }
        ...
        if (m_config.m_physicalApi == RigidBodyTwistControlComponentConfig::PhysicalApi::Force) {
            ...
            Physics::RigidBodyRequestBus::EventResult(currentLinearVelocityGlob, GetEntityId(),
                                                      &Physics::RigidBodyRequests::GetLinearVelocity);
            Physics::RigidBodyRequestBus::EventResult(currentAngularVelocityGlob, GetEntityId(),
            ...
            Physics::RigidBodyRequestBus::Event(GetEntityId(), &Physics::RigidBodyRequests::ApplyLinearImpulse,
                                                linearForceGlobal * fixedDeltaTime);
            Physics::RigidBodyRequestBus::Event(GetEntityId(), &Physics::RigidBodyRequests::ApplyAngularImpulse,
                                                angularForceGlobal * fixedDeltaTime);
