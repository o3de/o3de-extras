...
void ProximitySensor::Reflect(AZ::ReflectContext *context) {
  if (AZ::SerializeContext *serialize =
    ...
    if (AZ::EditContext *editContext = serialize->GetEditContext()) {
      editContext
          ->Class<ProximitySensor>("Proximity Sensor",
                                   "Proximity object detection component")
          ...
          ->DataElement(AZ::Edit::UIHandlers::Default,
                        &ProximitySensor::m_visualize, "Visualize",
                        "Whether the detection beam is shown")
          ->DataElement(AZ::Edit::UIHandlers::Default,
                        &ProximitySensor::m_frequency, "Frequency",
                        "Detection frequency")
          ->DataElement(AZ::Edit::UIHandlers::Default,
  ...
  if (AZ::BehaviorContext *behaviorContext =
          azrtti_cast<AZ::BehaviorContext *>(context)) {
    behaviorContext
        ->EBus<ProximitySensorNotificationBus>("ProximitySensor",
                                               "ProximitySensorNotificationBus")
        ->Attribute(AZ::Edit::Attributes::Category,
                    "WarehouseAutomation/ProximitySensor")
...
void ProximitySensor::Activate() {
  ...
  AZ::TransformBus::EventResult(entityTransform, GetEntityId(),
                                &AZ::TransformBus::Events::GetWorldTM);

  m_detectionDirection =
      entityTransform.TransformVector(AZ::Vector3::CreateAxisX());
  m_detectionDirection.Normalize();

  AZ::TickBus::Handler::BusConnect();
}

void ProximitySensor::Deactivate() { AZ::TickBus::Handler::BusDisconnect(); }
...
void ProximitySensor::Visualize() {
  if (m_drawQueue) {
    ...
    if (m_position) {
      ...
      linePoints.push_back(entityTranslation +
                           m_detectionDirection * m_detectionDistance);
    }
...
void ProximitySensor::OnTick(float deltaTime, AZ::ScriptTimePoint time) {
  ...
  AZ_Assert(m_frequency > 0.f,
            "ProximitySensor frequency must be greater than zero");
  ...
  if (deltaTime > frameTime) { // Frequency higher than possible, not catching
...
void ProximitySensor::DetectionCheck() {
  ...
  AZ::Vector3 entityTranslation;
  AZ::TransformBus::EventResult(entityTranslation, GetEntityId(),
                                &AZ::TransformBus::Events::GetWorldTranslation);
  ...
  if (AzPhysics::SceneHandle sceneHandle =
    ...
    m_position = !result.m_hits.empty()
                     ? std::make_optional(result.m_hits.front().m_position)
                     : std::nullopt;

    if (m_position) {
      ProximitySensorNotificationBus::Event(
          GetEntityId(), &ProximitySensorNotifications::OnObjectInRange);
    } else {
      ProximitySensorNotificationBus::Event(
          GetEntityId(), &ProximitySensorNotifications::OnObjectOutOfRange);
    }
  }
}
} // namespace WarehouseAutomation
