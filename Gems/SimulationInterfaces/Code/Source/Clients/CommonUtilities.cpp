...
namespace SimulationInterfaces::Utils
{
    ...
    AZ::Outcome<Bounds, AZStd::string> ConvertPhysicalShapeToBounds(const AZ::EntityId& entityId)
    {
        AzPhysics::ShapeColliderPairList physicalShapesConfigs;
        AZStd::vector<AZStd::shared_ptr<Physics::Shape>> physicalShapes;
        PhysX::ColliderComponentRequestBus::EventResult(
            physicalShapesConfigs, entityId, &PhysX::ColliderComponentRequests::GetShapeConfigurations);
        PhysX::ColliderComponentRequestBus::EventResult(physicalShapes, entityId, &PhysX::ColliderComponentRequests::GetShapes);

        if (physicalShapes.empty() && physicalShapesConfigs.empty())
        {
            return AZ::Failure("Physical body needs to have colliders");
        }
        // implementation supports bounds build from only one collider shape
        AZStd::shared_ptr<Physics::ShapeConfiguration> config = physicalShapesConfigs.front().second;
        const auto shape = physicalShapes.front();
