...
namespace ROS2
{
            ...
            if (AZ::EditContext* ec = serializeContext->GetEditContext())
            {
                ec->Class<LidarSensorConfiguration>("Lidar Sensor configuration", "Lidar sensor configuration")
                    ...
                    ->Attribute(
                        ...
                        { LidarSegmentationClassConfiguration::UnknownClass(),
                          LidarSegmentationClassConfiguration::GroundClass() }))
    ...
    AZ::Crc32 LidarSensorConfiguration::SegmentationClassesChangeNotify()
    {
        ...
        bool changed = false;
        const LidarSegmentationClassConfiguration unknownClass = LidarSegmentationClassConfiguration::UnknownClass();
        const LidarSegmentationClassConfiguration groundClass = LidarSegmentationClassConfiguration::GroundClass();
        for (auto& m_segmentationClasse : m_segmentationClasses)
        {
            if (m_segmentationClasse.m_className == unknownClass.m_className)
            {
                if (m_segmentationClasse.m_classId != unknownClass.m_classId)
                {
                    m_segmentationClasse.m_classId = unknownClass.m_classId;
                    changed = true;
                }

                unknown_present = true;
            }
            if (m_segmentationClasse.m_className == groundClass.m_className)
            {
                if (m_segmentationClasse.m_classId != groundClass.m_classId)
                {
                    m_segmentationClasse.m_classId = groundClass.m_classId;
                    changed = true;
                }
                ground_present = true;
            }
        ...
        if (!unknown_present)
        {
            m_segmentationClasses.push_back({ unknownClass.m_className, unknownClass.m_classId, unknownClass.m_classColor });
        }
        if (!ground_present)
        {
            m_segmentationClasses.push_back({ groundClass.m_className, groundClass.m_classId, groundClass.m_classColor });
        }
        if (changed)
        {
