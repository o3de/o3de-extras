# This script is used to convert JointsManipulationEditorComponent to use new Initial positions format in .prefab files.
...
def find_and_replace(data):
    def search_for_components(item, foundComponents):
        if isinstance(item, dict):
            ...
            if "JointsManipulationEditorComponent" in item and foundComponents:
                initialPositions = item["JointsManipulationEditorComponent"]["Initial positions"]
                if isinstance(initialPositions, dict):
                    positionsList = []
                    for jointName, position in initialPositions.items():
                        positionObject = {
                            "Name": jointName,
                            "Position": position
                        }
                        positionsList.append(positionObject)
                    item["JointsManipulationEditorComponent"]["Initial positions"] = positionsList
