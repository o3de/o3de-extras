...
namespace ROS2::Utils::xacro
{
        ...
        const AZStd::string& filename, const Params& params, const sdf::ParserConfig& parserConfig, const SdfAssetBuilderSettings& settings)
    {
        ...
        if (AZStd::unique_ptr<AzFramework::ProcessWatcher> xacroHelpProcess(AzFramework::ProcessWatcher::LaunchProcess(
            ...
            xacroHelpProcess != nullptr)
        {
            ...
            if (exitCode != 0)
            {
                const auto fullProcessCommand = AZStd::string::format(
                    "%s %s",
                    processLaunchInfo.m_processExecutableString.c_str(),
                    processLaunchInfo.GetCommandLineParametersAsString().c_str());
                AZ_Warning("ParseXacro", false, "Attempted to run validate xacro existence with command: %s", fullProcessCommand.c_str());
