...
namespace ROS2RobotImporter
{
    ...
    AZStd::string SdfAssetBuilder::GetFingerprint() const
    {
        ...
        AZ_Assert(saveObjectResult.IsSuccess(), "Failed to save settings to fingerprint string: %s", saveObjectResult.GetError().c_str());

        return settingsString;
    }

    void SdfAssetBuilder::CreateJobs(const AssetBuilderSDK::CreateJobsRequest& request, AssetBuilderSDK::CreateJobsResponse& response) const
    {
        ...
        if (!parsedSdfRootOutcome)
        {
            ...
            AZ_Error(
                SdfAssetBuilderName,
                false,
                R"(Failed to parse source file "%s". Errors: "%s")",
                fullSourcePath.c_str(),
                sdfParseErrors.c_str());
            return;
        }
    ...
    void SdfAssetBuilder::ProcessJob(const AssetBuilderSDK::ProcessJobRequest& request, AssetBuilderSDK::ProcessJobResponse& response) const
    {
        ...
        if (!saveResult)
        {
            AZ_Error(SdfAssetBuilderName, false, "Failed to write out temp asset file '%s'.", tempAssetOutputPath.c_str());
            response.m_resultCode = AssetBuilderSDK::ProcessJobResult_Failed;
            return;
        }
