...
namespace ROS2
{
    ...
    void SdfAssetPathResolverSettings::Reflect(AZ::ReflectContext* context)
    {
        if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            ...
            {
                editContext->Class<SdfAssetPathResolverSettings>("Asset Paths", "Exposes settings for resolving asset path references")
                    ->DataElement(
            ...
            if (auto editContext = serializeContext->GetEditContext(); editContext != nullptr)
            {
                editContext
                    ...
                    ->DataElement(
                        ...
                        "When set, fixes the URDF file before importing it. This is useful for fixing URDF files that have missing "
                        "inertials or duplicate names within links and joints.")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
    ...
    void SdfAssetBuilderSettings::LoadSettings(AZ::SettingsRegistryInterface* settingsRegistry)
    {
        ...
        auto VisitFileTypeExtensions = [&settingsRegistry, this](const AZ::SettingsRegistryInterface::VisitArgs& visitArgs)
        {
            if (AZ::SettingsRegistryInterface::FixedValueString value; settingsRegistry->Get(value, visitArgs.m_jsonKeyPath))
            {
                ...
                AZStd::string wildcardPattern =
                    value.starts_with('.') ? AZStd::string::format("*%s", value.c_str()) : AZStd::string::format("*.%s", value.c_str());

                m_builderPatterns.push_back(
                    AssetBuilderSDK::AssetBuilderPattern(wildcardPattern, AssetBuilderSDK::AssetBuilderPattern::PatternType::Wildcard));
            }
            return AZ::SettingsRegistryInterface::VisitResponse::Continue;
        };
        AZ::SettingsRegistryVisitorUtils::VisitArray(
            *settingsRegistry, VisitFileTypeExtensions, SdfAssetBuilderSupportedFileExtensionsRegistryKey);
