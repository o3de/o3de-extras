...
{
    AZ_COMPONENT_IMPL(GeoreferencingEditorSystemComponent, "GeoreferencingEditorSystemComponent",
        GeoreferencingEditorSystemComponentTypeId, BaseSystemComponent);

    void GeoreferencingEditorSystemComponent::Reflect(AZ::ReflectContext* context)
    {
        if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            serializeContext->Class<GeoreferencingEditorSystemComponent, GeoreferencingSystemComponent>()
                ->Version(0);
        }
    }

    GeoreferencingEditorSystemComponent::GeoreferencingEditorSystemComponent() = default;
