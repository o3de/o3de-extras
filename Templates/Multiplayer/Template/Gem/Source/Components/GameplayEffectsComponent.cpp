...
#include <Source/Components/GameplayEffectsComponent.h>
    ...
    void GameplayEffectsComponent::SpawnEffect(SoundEffect effect, const AZ::Vector3& position)
    {
        ...
        if (!triggerName || strlen(triggerName) == 0)
        {
            if (mps_enableMissingAudioTriggerWarnings)
            {
                ...
                AZ_Warning("${SanitizedCppName}", false, "Audio trigger wasn't specified for effect [%s] on GameplayEffectsComponent", eventName.c_str());
            }
            return;
        }
