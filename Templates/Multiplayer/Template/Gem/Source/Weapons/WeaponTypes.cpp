...
namespace ${SanitizedCppName}
{
    ...
    void ReflectWeaponEnums(AZ::ReflectContext* context)
    {
        ...
        if (serializeContext)
        {
            ...
            if (editContext)
            {
                editContext->Enum<SoundEffect>("SoundEffect", "Valid sound effects to use within ${SanitizedCppName}")
                ...
                editContext->Enum<CharacterAnimState>("CharacterAnimState", "Various ${SanitizedCppName} character animation states")
                ...
                editContext->Enum<WeaponType>("WeaponType", "Different weapon types in ${SanitizedCppName}")
                    ->Value("None", WeaponType::None)
                    ->Value("Trace", WeaponType::Trace)
                    ->Value("Projectile", WeaponType::Projectile);

                editContext->Enum<WeaponStatus>("WeaponStatus", "Status states for a ${SanitizedCppName} weapon")
    ...
    void HitEntity::Reflect(AZ::ReflectContext* context)
    {
        ...
        if (behaviorContext)
        {
            behaviorContext->Class<HitEntity>("HitEntity")
                ...
                ->Attribute(AZ::Script::Attributes::Module, "${SanitizedCppName}")
                ->Attribute(AZ::Script::Attributes::Category, "${SanitizedCppName}")
                ->Constructor<>()
